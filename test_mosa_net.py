...
def test_model(model, test_loader, config, class_names=['CN', 'AD']):
    ...
    # ROC 图（支持：二分类直接；多分类 micro-average）
    try:
    ...
    if getattr(getattr(config, 'logging', None), 'export_per_class', True):
        try:
            ...
            pass
    # 保存 summary_all.json（仅测试脚本版本：无 best_val，用占位）
    try:
        run_end = datetime.datetime.now(datetime.timezone.utc)
        run_start = getattr(config, '_run_start_time', run_end)  # 若 main() 中设定，可覆盖
...
def main():
    ...
    config.training.resume = "/path/to/mosa_trained_weights.pth"  # 替换为Mosa训练权重路径
    ...
    if config.training.resume:
        print(f"加载Mosa训练权重: {config.training.resume}")
